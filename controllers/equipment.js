...
equipmentController.list = async (req, res, next) => {
    try {
        ...
        const products = await EquipmentModel.find().populate('logo').populate('type');
...
equipmentController.edit = async (req, res, next) => {
    try {
        ...
        let product = await EquipmentModel.findById(productId);
        if (product == null) {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const typeFind = await EquipmentTypeModel.findById(type);
                const productSaved = await EquipmentModel.findByIdAndUpdate(productId, {
                    name: name,
                    code: code,
                    description: description,
                    serialNumber: serialNumber,
...
equipmentController.show = async (req, res, next) => {
    try {
        ...
        let product = await EquipmentModel.findById(productId).populate('logo');
        if (product == null) {
            return res.status(httpStatus.NOT_FOUND).json({
                message: "Không tìm thấy sản phẩm"
            });
...
equipmentController.delete = async (req, res, next) => {
    try {
        ...
        let productType = await EquipmentModel.findByIdAndDelete(productTypeId);
        if (productType == null) {
            return res.status(httpStatus.NOT_FOUND).json({
